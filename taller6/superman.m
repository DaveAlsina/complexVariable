...
sinruido = fillmissing(sinruido,'previous');
conruido = fillmissing(conruido,'previous');

%% Análisis del ruido
% Análisis por cada eje
visualize_XY_noisy_signal(x, newx, y, newy, t);
% Análisis en el planoparametrizacion
%visualize_noisy_signal(sinruido, conruido, ['Supermán (Ruido 50% más fuerte)']);
grid on;
    axis equal;
    ...
    hold on;
        plot(x,y, '-k');
        plot(newx,newy, '-.b');
        title('Supermán Ruidoso');            %añade el título a la imagen
...
z_fft = fft(conruido);
PSD = 2.*abs(z_fft/length(t)); 
indices = PSD > 1.2;
coef = indices.*z_fft;
reconstruccion2 = ifft(coef);
plot(reconstruccion2, '-r');
...
hold on
    ...
    plot(reconstruccion2, '-r');
    title(['Señal de Supermán reconstruida']);
    legend(['Filtro +'],['Filtro -']);
hold off

visualize_XY_noisy_signal(x, real(reconstruccion1), y, imag(reconstruccion1), t);
    
% Fuentes
% https://www.desmos.com/calculator/vzkbi5ttgg